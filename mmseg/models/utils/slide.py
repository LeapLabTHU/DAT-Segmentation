...
class SlideAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(self.rpb, self.padding_mode, self.share_dwc_kernel, self.share_qkv)
        B, H, W, C = x.shape
        qkv = self.qkv(x)

        # # fully connected layer
        # f_all = qkv.reshape(B, H*W, self.qkv_scale*self.num_heads, C//self.num_heads).permute(0, 2, 1, 3) # B, 3*nhead, H*W, C//nhead
        # f_conv = self.fc(f_all).reshape(B*self.num_heads, 2+1, H*W, C//self.num_heads).permute(0, 3, 1, 2).reshape(B*self.num_heads, (2+1)*C//self.num_heads, H, W)
